...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        async function fetchAppointments() {
            ...
            var apiUrl = '/api/appointment/agent';
            var parameters = {
                method: 'GET',
                headers: {
            ...
            if (data.length == 0) {
                ...
                data.forEach(appointment => {
                    ...
                    <tr>
                        ...
                        <td>${appointment.client.firstName} ${appointment.client.middleName} ${appointment.client.lastName}</td>
                        <td>
                            <abbr title="view"><i class="bi bi-eye-fill px-1" data-bs-toggle="modal" data-bs-target="#moreInfo${appointment.id}"></i></abbr>
                            <abbr title="confirm appointment"><i class="bi bi-pencil-fill px-1" data-bs-toggle="modal" data-bs-target="#confirm${appointment.id}"></i></abbr>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="container m-3">
                            <div class="row">
                                ...
                                <div class="col-lg-6">
                                    <p><strong>Agent: </strong>${info.client.firstName} ${info.client.middleName} ${info.client.lastName}</p>
                                </div>
                                <div class="col-lg-6">
                                    <p><strong>Contact: </strong>${info.client.phoneNo}</p>
                                </div>
                                <div class="col-lg-6">
                                    <p><strong>Email: </strong>${info.client.email}</p>
                                </div>
                                <hr>
                                <div>
                                    <h4>Appointment</h4>
        ...
        function confirmModalMaker(data) {
            if (data.length > 0) {
                var modalContainer = document.querySelector('.confirm-modals');
                if (!modalContainer) return;
                ...
                data.forEach(confirm => {
                    modalsHTML += `
        <div class="modal fade" id="confirm${confirm.id}" tabindex="-1">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <form class="confirm-appointment" data-item-id="${confirm.id}">
                            <div class="m-3">  
                                <h5>Location</h5>
        ...
        async function updateAppointment(formData, id) {

        }

        setTimeout(() => {
            var confirmAppointment = document.querySelectorAll('.confirm-appointment');

            confirmAppointment.forEach(function(appointment) {
                appointment.addEventListener('submit', async function(event) {

                    event.preventDefault();

                    var formData = new FormData(appointment);
                    var id = appointment.getAttribute('data-item-id');

                    var apiUrl = '/api/appointment/schedule/' + id;
                    var parameters = {
                        method: 'POST',
                        headers: {
                            'Accept': 'application/json',
